...
# ─── 물품 조사 현황 ───
INVENTORY_ITEMS = [
    {'key': 'a4',         'name': 'A4용지',       'unit': '팩',    'threshold': 4},
    {'key': 'roll_tissue','name': '롤 휴지',      'unit': '롤',    'threshold': 10},
    {'key': 'wet_wipe',   'name': '물티슈',       'unit': '개',    'threshold': 3},
    {'key': 'stapler',    'name': '스테이플러 심','unit': '통',    'threshold': 0},
    {'key': 'water_cup',  'name': '정수기 물컵',  'unit': '줄',    'threshold': 10},
    {'key': 'trash_bag',  'name': '종량제 봉투',  'unit': '봉투',  'threshold': 1},
    {'key': 'chalk',      'name': '흰 분필',      'unit': '개',    'threshold': 20},
]
INVENTORY_BRANCHES = ['서천', '청명']
...
def inventory():
    db = get_db()
    branches = []
    for bname in INVENTORY_BRANCHES:
        row = db.execute('SELECT * FROM branch WHERE name = ?', (bname,)).fetchone()
        if row:
            branches.append({'id': row['id'], 'name': bname})
        else:
            branches.append({'id': None, 'name': bname})

    rows = db.execute('SELECT branch_id, item_key, quantity FROM inventory_item').fetchall()
    qty_map = {(r['branch_id'], r['item_key']): r['quantity'] for r in rows}
    db.close()
    ...
    for item in INVENTORY_ITEMS:
        ...
        total = 0
        for b in branches:
            q = qty_map.get((b['id'], item['key']), 0) if b['id'] else 0
            total += q
            per_branch.append({
                'branch': b['name'],
                'branch_id': b['id'],
                'quantity': q,
                'low': q <= item['threshold'],
            })
        data.append({
            'item': item,
            'branches': per_branch,
            'total': total,
            'total_low': total <= item['threshold'],
        })

    return render_template('inventory.html', data=data, branches=branches, items=INVENTORY_ITEMS)
...
def inventory_save():
    db = get_db()
    for bname in INVENTORY_BRANCHES:
        brow = db.execute('SELECT id FROM branch WHERE name = ?', (bname,)).fetchone()
        if not brow:
            continue
        bid = brow['id']
        for item in INVENTORY_ITEMS:
            field = f"qty_{bid}_{item['key']}"
            ...
            try:
                ...
                continue
            db.execute('''
                INSERT INTO inventory_item (branch_id, item_key, quantity, updated_at)
                VALUES (?, ?, ?, CURRENT_TIMESTAMP)
                ON CONFLICT(branch_id, item_key)
                DO UPDATE SET quantity = ?, updated_at = CURRENT_TIMESTAMP
            ''', (bid, item['key'], q, q))
    db.commit()
    db.close()
    flash('물품 조사 현황이 저장되었습니다.')
